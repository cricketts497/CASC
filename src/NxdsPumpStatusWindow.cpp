...
// nominal_speed(30)
{
    ...
    for(int i=0; i<pump_names.length(); i++){
        ...
        QELabel * speed = new QELabel(QString("CASC:BL:%1:Speed").arg(pump_names.at(i).mid(2)), this);
        QELabel * temperature = new QELabel(QString("CASC:BL:%1:Temperature").arg(pump_names.at(i).mid(2)), this);
        QELabel * status = new QELabel(QString("CASC:BL:%1:Status").arg(pump_names.at(i).mid(2)), this);
        QELabel * service = new QELabel(QString("CASC:BL:%1:Service").arg(pump_names.at(i).mid(2)), this);
        ...
        layout->addWidget(speed,1,i+1);
        layout->addWidget(temperature,2,i+1);
        layout->addWidget(status,3,i+1);
        layout->addWidget(service,4,i+1);
    }
    
    widget->setFixedSize(700,240);
}
